...
public class SourceQuery extends HttpServlet
{
    ...
    private void queryList(final HttpServletRequest request, final HttpServletResponse response) throws IOException {
        ...
        String sql = "";
        final String sqlf = "select id,sourcename from projectsource where 1=1";
        final String sqle = "  order by id limit " + offset + "," + limit;
        String sql2 = "select count(*) numbers from projectsource where 1=1  ";
        String str = "";
        if (id != "" && id != null) {
            str = String.valueOf(str) + " and id like '%" + id + "%'";
        }
        if (name != "" && name != null) {
            str = String.valueOf(str) + " and sourcename like '%" + name + "%'";
        }
    ...
    private void deletesingle(final HttpServletRequest request, final HttpServletResponse response) throws IOException {
        ...
        final PrintWriter out = response.getWriter();
        final Db db = new Db();
        final String sql = "delete from projectsource where id = ?";
        final int id = Integer.parseInt(request.getParameter("id"));
        final String sql2 = "select * from paper where prosourceid = ?";
