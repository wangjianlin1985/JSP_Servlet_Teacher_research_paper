...
public class FsEdit extends HttpServlet
{
    ...
    protected void doPost(final HttpServletRequest request, final HttpServletResponse response) throws ServletException, IOException {
        ...
        final String firstsubname = json.getString("firstsubname");
        final int partid = json.getInt("partid");
        Label_0388: {
            if (id != oldid) {
                String sql = "select * from firstsub where id= ?";
            ...
            String sql = "update firstsub set id = ?,firstsubname = ?,subpartid=? where id = ?";
            PreparedStatement ps = db.getPs(sql);
            try {
                ps.setInt(1, id);
                ps.setString(2, firstsubname);
