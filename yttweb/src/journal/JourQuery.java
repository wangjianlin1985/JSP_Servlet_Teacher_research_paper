...
public class JourQuery extends HttpServlet
{
    ...
    private void queryList(final HttpServletRequest request, final HttpServletResponse response) throws IOException {
        ...
        final String name = request.getParameter("journalname");
        ...
        String sql = "";
        final String sqlf = "select journal.id,journalname,pubpartname from journal,pubpart where (journal.pubpartid = pubpart.id)";
        final String sqle = "  order by journal.id limit " + offset + "," + limit;
        String sql2 = "select count(*) numbers from journal,pubpart where (journal.pubpartid = pubpart.id)  ";
        String str = "";
        if (id != "" && id != null) {
            str = String.valueOf(str) + " and journal.id like '%" + id + "%'";
        }
        if (name != "" && name != null) {
            str = String.valueOf(str) + " and journalname like '%" + name + "%'";
        }
        if (partid != "" && partid != null) {
            str = String.valueOf(str) + " and pubpartid like '%" + partid + "%'";
        }
    ...
    private void deletesingle(final HttpServletRequest request, final HttpServletResponse response) throws IOException {
        ...
        final PrintWriter out = response.getWriter();
        final Db db = new Db();
        final String sql = "delete from journal where id = ?";
        final int id = Integer.parseInt(request.getParameter("id"));
        final String sql2 = "select * from paper where journalid = ?";
