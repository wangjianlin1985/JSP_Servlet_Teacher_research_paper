...
public class PubQuery extends HttpServlet
{
    ...
    private void deletesingle(final HttpServletRequest request, final HttpServletResponse response) throws IOException {
        ...
        final Db db = new Db();
        final String sql = "delete from pubpart where id = ?";
        final int id = Integer.parseInt(request.getParameter("id"));
        final String sql2 = "select id from paper where pubtypeid = ?  UNION select id from journal where pubpartid = ?";
    ...
    private void queryList(final HttpServletRequest request, final HttpServletResponse response) throws IOException {
        ...
        final String name = request.getParameter("pubpartname");
        ...
        String sql = "";
        final String sqlf = "select id,pubpartname,grade from pubpart where 1=1";
        final String sqle = "  order by id limit " + offset + "," + limit;
        String sql2 = "select count(*) numbers from pubpart where 1=1  ";
        String str = "";
        if (id != "" && id != null) {
            str = String.valueOf(str) + " and id like '%" + id + "%'";
        }
        if (name != "" && name != null) {
            str = String.valueOf(str) + " and pubpartname like '%" + name + "%'";
        }
        ...
        final List<PubBean> PubList = new ArrayList<PubBean>();
        ...
        try {
            ...
            while (rs2.next()) {
                final PubBean pubbean = new PubBean();
                pubbean.setId(rs2.getInt(1));
                pubbean.setName(rs2.getString(2));
                pubbean.setGrade(rs2.getInt(3));
                PubList.add(pubbean);
            }
